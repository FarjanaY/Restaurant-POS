...
test('paginates and filters the order list by type, returning total/page/limit metadata', async () => {
  for (let i = 0; i < 3; i += 1) {
  ...
  const res = await fetch(`${baseUrl}/api/orders?status=open&type=dine_in&page=1&limit=2`, {
    headers: cashierHeader,
  });
  assert.equal(res.status, 200);
  ...
  assert.ok(body.orders.every((o) => o.type === 'dine_in' && o.status === 'open'));
});

test('searches the order list by token number', async () => {
  const createRes = await fetch(`${baseUrl}/api/orders`, {
    method: 'POST',
    headers: cashierHeader,
    body: JSON.stringify({ type: 'takeaway', lines: [{ menuItemId: coldSandwich._id, quantity: 1 }] }),
  ...
  const res = await fetch(`${baseUrl}/api/orders?search=${order.tokenNumber}&page=1&limit=10`, {
    headers: cashierHeader,
  });
  const body = await res.json();
